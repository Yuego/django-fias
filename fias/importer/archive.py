...
class Archive(object):
    ...
    def __init__(self, version=None, path=None):
        ...
        self._archive = None
        self._date = None
        self._progress = DlProgressBar(max=100)
        self._retrieve(version, path)

    def update_progress(self, count, block_size, total_size):
        self._progress.goto(int(count * block_size * 100 / total_size))
    ...
    def _retrieve(self, version=None, path=None):
        ...
        if self._path is None:
            ...
            self._path = urlretrieve(path, reporthook=self.update_progress)[0]
